<template>
  <div class="date_panel f_disselected" v-if="panelStatus" @mouseover="clearPanelTimer" @mouseout="hidePanel" @click="hidePickerNow">
    <div class="panel_head">
      ...
      <span class="change_month" @click="changeMonth(1)">&gt;</span>
      <div class="choose_box" :class="{'choose_year_box':changeType === 'y'}" @mouseover="clearPickerTimer" @mouseout="hidePicker" v-show="showChooseBox">
        <p class="change_year" v-if="changeType === 'y'">
          <span @click="changeYearPage" changeval="-8">&lt;</span>
          <span @click="changeYearPage" changeval="8">&gt;</span>
        </p>
        <span class="item" v-for="(item,index) in items" :key="index" @click="chooseYearMonth(item)">{{item}}</span>
      </div>
    </div>
    <div class="panel_body">
      <p class="weekday_box">
        <span v-for="(weekday,index) in weekdayArry" :key="index" class="weekday">{{ weekday }}</span>
      </p>
      <div class="day_box">
        <span v-for="(item,index) in days" :key="index" :class="{'crn_month':item.isCurMonth,'only_day': item.isChoosed && item.isCurMonth}" @click="choosDay(item.dayNum,item.isCurMonth)">{{ item.dayNum }}</span>
      </div>
    </div>
  </div>
</template>
